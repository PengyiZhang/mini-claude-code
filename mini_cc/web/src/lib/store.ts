...
export function rawToChatMessages(raw: RawMessage[]): ChatMessage[] {
  ...
  // When folding a tool_result back into its owner, we need to know
  // whether it belongs to a real activity or to a synthetic __card__
  // tool_use. Returns "card" if the id matches a card block (so the
  // caller can skip the result — its content is just "ok"), or
  // "activity" otherwise.
  const findOwner = (msg: ChatMessage, id: string): "card" | "activity" => {
    if (msg.cards?.some((c) => id.startsWith("card_"))) {
      // Synthetic card tool_use ids have the prefix "card_"; that's
      // enough signal here since real tool_use ids are generated by
      // the LLM and won't collide with this prefix.
      // (Refined check: we don't store the tool_use id on the card,
      // so we rely on the prefix convention.)
  ...
  for (let i = 0; i < raw.length; i++) {
    ...
    for (const b of toolUses) {
      // Synthetic __card__ blocks: route to msg.cards instead of
      // msg.activities. The block's `input` IS the card payload — we
